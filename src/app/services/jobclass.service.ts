...
export class JobClassService {
    ...
    getJobClassList() {
        return this.http.get<any>(`${this.config.apiEndpoint}/JobClass/GetJobClass`).pipe(map((data: any) => {
            return data;
        }));
    }
    setJobClass(jobClass:jobClassModel):Observable<any>{
        return this.http.post<any>(`${this.config.apiEndpoint}/JobClass`,jobClass);
    }
